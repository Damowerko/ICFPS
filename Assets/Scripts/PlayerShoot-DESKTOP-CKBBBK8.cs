...
#pragma warning disable 169
    ...
    private IEnumerator Ballisitcs()
    {
        ...
        while (true)
        {
            ...
            if (velocity.magnitude < 1 || Time.time - initialTime > weapon.bulletLifetime)
                yield break;
            ...
            if (currentPos != oldPos)
            {
                ...
                if (Physics.Linecast(oldPos, currentPos, out _hit))
                {
                    ...
                    else // if the bullet stops dead
                    {
                        ...
                        if (_hit.collider.transform.parent != null && chunk == null)
                           chunk = _hit.collider.transform.parent.GetComponent<FracturedChunk>();
    ...
    private IEnumerator TracerBallistics(Vector3 _velocity, Vector3 _startPosition)
    {
        ...
        while (true)
        {
            ...
            //the coroutine will terminate when the velocity is to small to do damage or bullet lifetime has expired
            if (_velocity.magnitude < 1 || Time.time - initialTime > weapon.bulletLifetime)
            {
                Destroy(tracerClone);
                yield break;
            }
